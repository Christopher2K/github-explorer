...
class OperationResponseParser extends DefaultResponseParser
{
    ...
    protected function handleParsing(CommandInterface $command, Response $response, $contentType)
    {
        ...
        if (!$model) {
            // Return basic processing if the responseType is not models or the models cannot be found
            ...
            // Returns a models with no visiting if the command response processing is not models
            return new Model(parent::handleParsing($command, $response, $contentType));
        } else {
            // Only inject the schema into the models if "schemaInModel" is true
        ...
        if ($location = $additional->getLocation()) {
            ...
            if (is_array($result)) {
                ...
                foreach (array_keys($result) as $key) {
                    // Check if the models actually knows this property. If so, then it is not additional
